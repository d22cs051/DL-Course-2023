...
def get_url_from_gdrive_confirmation(contents):
    ...
    for line in str(contents).splitlines():
        ...
        if m:
            ...
            url = url.replace('&amp;', '&')
            print(f"[DEBUG] ret 1. url: {url}")
            return url
        m = re.search('confirm=([^;&]+)', line)
        if m:
            soup = bs4(line, 'html.parser').find_all('form')
            # print(f"[DEBUG] soup: {soup[0].get('action')}")
            # print(f"[DEBUG] m.groups(): {m.groups()}")
            confirm = m.groups()[0]
            # print(f"[DEBUG] confirm: {confirm}")
            url = soup[0].get('action')
            # url = url[:url.rfind('&')]
            url = re.sub(r'confirm=([^;&]+)', r'confirm='+confirm, url)
            print(f"[DEBUG] ret 2. url: {url}")
            return url
...
def download(url, output, quiet):
    ...
    while True:
        ...
        print(f"[DEBUG in download] url: {url}")

        if url is None:
            print(f'Permission denied: {url_origin}')
